...
function validateUname()
{
	...
	if(!value)
	{
		$("#"+id+"Error").text("用户名不能为空");//获取label 添加错误信息 
	...
	{
		$("#"+id+"Error").text("请输入3-30位用户名");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	return true;
...
function validateUpwd()
{
	...
	if(!value)
	{
		$("#"+id+"Error").text("密码不能为空");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	//长度校验
	...
	{
		$("#"+id+"Error").text("请输入6-30位密码");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	return true;
...
function validateAffpwd()
{
	...
	if(!value)
	{
		$("#"+id+"Error").text("确认密码不能为空");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	//校验两次输入是否一致
	if(value != $("#upwd").val()) {
		$("#" + id + "Error").text("两次输入不一致");//获取label 添加错误信息 
		showError($("#" + id + "Error"));//显示label
		false;
	}
	return true;
...
function validateEmail()
{
	...
	if(!value)
	{
		$("#"+id+"Error").text("email不能为空");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	//格式校验
	...
	{
		$("#"+id+"Error").text("错误的email格式");//获取label 添加错误信息 
		showError($("#"+id+"Error"));//显示label
		return false;
	}
	return true;
...
function checkUname(){
	...
	//打开连接
	xhr.open("GET","${pageContext.request.contextPath}/user_findByUname.action?time="+new Date().getTime()+"&uname="+uname,true);//
	//发送
	xhr.send(null);
}
